...
var uuid = require('uuid');  // show errors in console without killing app
...
function create_person (request, reply) {
  ...
  var phone = help.escape(request.payload.phone);
  var sex = help.escape(request.payload.sex);
  var select = escape('SELECT * FROM "Patient" WHERE (patient_email = %L)', email);
...
module.exports = function register_handler(request, reply, source, error) {
  if (request.method === 'get') { // get does not send payload so return reg
    return reply({ title : 'Please Register' }).code(200);
  }
  // console.log(error);
